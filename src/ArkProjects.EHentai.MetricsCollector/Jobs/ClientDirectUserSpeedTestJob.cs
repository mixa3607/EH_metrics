...
public class ClientDirectUserSpeedTestJob : IJob
{
    private readonly ILogger<CollectHomeOverviewMetricsJob> _logger;
    private readonly MetricsCollectorService _metricsCollector;

    public ClientDirectUserSpeedTestJob(ILogger<CollectHomeOverviewMetricsJob> logger,
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        var tasks = Enumerable.Range(0, threads).Select(async _ =>
        {
            var request = HathWorkerApi
                .GetServerSpeedTestRequest(host, unixTime, size, clientId, clientKey)
                .WithTimeout(10_000);
        ...
        else
        {
            ...
            var kbps = (int)(size * threads / elapsed.TotalSeconds / 1024);
